...
size_t Buffer_ICDE16::getMemoryUsage(bool includeSecAttrs) {
    size_t totalSize = sizeof(size_t);

    totalSize += sizeof(*entries);
    totalSize += entries->size() * (sizeof(RecordId) + sizeof(pair<RecordId, Timestamp>));

    if (includeSecAttrs) {
        totalSize += sizeof(*secAttrs);
        totalSize += secAttrs->size() * (sizeof(RecordId) + sizeof(pair<RecordId, int>));
    }
...
void Buffer_ICDE16::execute_gOverlaps(RangeQuery Q, vector<RecordId> &candidates)
{
    auto iter = this->entries->begin(), iterEnd = this->entries->end();
    while (iter != iterEnd)
    {
...
void Buffer_ICDE16::execute_gOverlaps(vector<RecordId> &candidates)
{
    auto iter = this->entries->begin(), iterEnd = this->entries->end();
    while (iter != iterEnd)
    {
...
void* Buffer_ICDE16::getValuesArray()
{
    return (void*)this->entries->getValuesArray();
}
