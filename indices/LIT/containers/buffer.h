...
public:
    ...
    virtual size_t getMemoryUsage(bool includeSecAttrs = false) { return 0; }
...
public:
    ...
    Timestamp remove_secAttr(RecordId id);
    size_t getSize();
    size_t getMemoryUsage(bool includeSecAttrs = false);
    void print(char c);
    void destroy();
    ~Buffer_ICDE16();
    ...
    size_t execute_gOverlaps(RangeQuery Q);
    size_t execute_gOverlaps();
    void execute_gOverlaps(RangeQuery Q, vector<RecordId> &candidates);
    void execute_gOverlaps(vector<RecordId> &candidates);
    ...
    // Sampling support - get values array pointer
    void* getValuesArray();
