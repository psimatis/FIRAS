...
size_t HINT_M_Dynamic::getSize()
{
    size_t size = sizeof(*this);

    size += pOrgsInIds.size() * sizeof(vector<vector<RecordId>>);
    size += pOrgsAftIds.size() * sizeof(vector<vector<RecordId>>);
    size += pRepsInIds.size() * sizeof(vector<vector<RecordId>>);
    size += pRepsAftIds.size() * sizeof(vector<vector<RecordId>>);
    size += pOrgsInTimestamps.size() * sizeof(vector<vector<pair<Timestamp, Timestamp>>>);
    size += pOrgsAftTimestamps.size() * sizeof(vector<vector<pair<Timestamp, Timestamp>>>);
    size += pRepsInTimestamps.size() * sizeof(vector<vector<pair<Timestamp, Timestamp>>>);
    size += pRepsAftTimestamps.size() * sizeof(vector<vector<pair<Timestamp, Timestamp>>>);

    for (auto l = 0; l < this->height; l++)
    {
        size += pOrgsInIds[l].size() * sizeof(vector<RecordId>);
        size += pOrgsAftIds[l].size() * sizeof(vector<RecordId>);
        size += pRepsInIds[l].size() * sizeof(vector<RecordId>);
        size += pRepsAftIds[l].size() * sizeof(vector<RecordId>);
        size += pOrgsInTimestamps[l].size() * sizeof(vector<pair<Timestamp, Timestamp>>);
        size += pOrgsAftTimestamps[l].size() * sizeof(vector<pair<Timestamp, Timestamp>>);
        size += pRepsInTimestamps[l].size() * sizeof(vector<pair<Timestamp, Timestamp>>);
        size += pRepsAftTimestamps[l].size() * sizeof(vector<pair<Timestamp, Timestamp>>);

        auto cnt = pow(2, this->numBits-l);
        
        for (int pid = 0; pid < cnt; pid++)
        {
            size += this->pOrgsInIds[l][pid].size() * sizeof(RecordId);
            size += this->pOrgsAftIds[l][pid].size() * sizeof(RecordId);
            size += this->pRepsInIds[l][pid].size() * sizeof(RecordId);
            size += this->pRepsAftIds[l][pid].size() * sizeof(RecordId);
            
            size += this->pOrgsInTimestamps[l][pid].size() * sizeof(pair<Timestamp, Timestamp>);
            size += this->pOrgsAftTimestamps[l][pid].size() * sizeof(pair<Timestamp, Timestamp>);
            size += this->pRepsInTimestamps[l][pid].size() * sizeof(pair<Timestamp, Timestamp>);
            size += this->pRepsAftTimestamps[l][pid].size() * sizeof(pair<Timestamp, Timestamp>);
        }
    }
...
void HINT_M_Dynamic::execute_pureTimeTravel(RangeQuery Q, vector<RecordId> &candidates) {
    vector<pair<Timestamp, Timestamp> >::iterator iter, iterStart, iterEnd;
    RelationIdIterator iterI, iterIStart, iterIEnd;
    Timestamp a = Q.start >> (this->maxBits-this->numBits); // prefix
    Timestamp b = Q.end   >> (this->maxBits-this->numBits); // prefix
