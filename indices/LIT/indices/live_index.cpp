...
template <class T>
size_t LiveIndexCapacityConstrainted<T>::getMemoryUsage(bool includeSecAttrs) {
    size_t totalSize = 0;

    for (auto& buffer : buffers) 
        totalSize += buffer.getMemoryUsage(includeSecAttrs);

    totalSize += offsets_starts.size() * sizeof(Timestamp);
    totalSize += offsets_ids.size() * sizeof(RecordId);

    return totalSize;
}

template <class T>
Timestamp LiveIndexCapacityConstrainted<T>::getEarliestTimestamp() {
    if (offsets_starts.empty()) 
        return 0;
    return offsets_starts[0];
}
...
template <class T>
void LiveIndexCapacityConstrainted<T>::execute_pureTimeTravel_candidates(RangeQuery Q, vector<DIT::DIT_CandidateLog> &candidates) {
    vector<Timestamp>::iterator iterO    = this->offsets_starts.begin();
    vector<Timestamp>::iterator iterOEnd = lower_bound(iterO, this->offsets_starts.end(), Q.end+1);
    typename vector<T>::iterator iterB   = this->buffers.begin();
    typename vector<T>::iterator pivot   = iterB+(iterOEnd-iterO-1);

    // For all buffers except the last one, create candidate logs with values array pointer
    while (iterB != pivot) {
        size_t bufferSize = iterB->getSize();
        if (bufferSize > 0)
            candidates.push_back(DIT::DIT_CandidateLog(iterB->getValuesArray(), LIVE_INDEX, 0, bufferSize));
        iterB++;
    }

    // Last buffer - collect candidates into vector
    vector<RecordId>* lastBufferCandidates = new vector<RecordId>();
    lastBufferCandidates->reserve(iterB->getSize());
    iterB->execute_gOverlaps(Q, *lastBufferCandidates);
    if (lastBufferCandidates->size() > 0)
        candidates.push_back(DIT::DIT_CandidateLog((void*)lastBufferCandidates, LIVE_INDEX_LAST_BUFFER, 0, lastBufferCandidates->size()));
}

template <class T>
void LiveIndexCapacityConstrainted<T>::execute_pureTimeTravel_candidates(RangeQuery Q, vector<AugmentedIntervalTree::AIT_CandidateLog> &candidates) {
    vector<Timestamp>::iterator iterO    = this->offsets_starts.begin();
    vector<Timestamp>::iterator iterOEnd = lower_bound(iterO, this->offsets_starts.end(), Q.end+1);
    typename vector<T>::iterator iterB   = this->buffers.begin();
    typename vector<T>::iterator pivot   = iterB+(iterOEnd-iterO-1);
    ...
    while (iterB != pivot) {
        size_t bufferSize = iterB->getSize();
        if (bufferSize > 0)
            candidates.push_back(AugmentedIntervalTree::AIT_CandidateLog(iterB->getValuesArray(), LIVE_INDEX, 0, bufferSize));
        iterB++;
    }

    // For the last buffer, collect all candidates in a vector
    vector<RecordId>* lastBufferCandidates = new vector<RecordId>();
    lastBufferCandidates->reserve(iterB->getSize());
    iterB->execute_gOverlaps(Q, *lastBufferCandidates);
    if (lastBufferCandidates->size() > 0)
        candidates.push_back(AugmentedIntervalTree::AIT_CandidateLog((void*)lastBufferCandidates, LIVE_INDEX_LAST_BUFFER, 0, lastBufferCandidates->size()));
}


template <class T>
void LiveIndexCapacityConstrainted<T>::execute_pureTimeTravel(RangeQuery Q, vector<RecordId> &candidates)
{
    vector<Timestamp>::iterator iterO    = this->offsets_starts.begin();
    vector<Timestamp>::iterator iterOEnd = lower_bound(iterO, this->offsets_starts.end(), Q.end+1);
    typename vector<T>::iterator iterB   = this->buffers.begin();
    ...
    // For all buffers except the last one, collect all candidates
    while (iterB != pivot)
    {
        iterB->execute_gOverlaps(candidates);
        iterB++;
    }

    // For the last buffer, collect candidates with query filtering
    iterB->execute_gOverlaps(Q, candidates);
}
