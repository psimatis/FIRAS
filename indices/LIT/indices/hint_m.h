...
// HINT^m with subs and cash misses optimizations activated
class HINT_M_Subs_CM : public HierarchicalIndex
{
private:
//    Relation      **pOrgsInTmp;
...
public:
    // Construction
    HINT_M_Subs_CM(const Relation &R, const unsigned int numBits, const unsigned int maxBits);
    void getStats();
    ~HINT_M_Subs_CM();
...
// Dynamic HINT^m with subs and cash misses optimizations activated
class HINT_M_Dynamic_Naive : public HierarchicalIndex
{
private:
    vector<pair<RecordId, Timestamp>> aliveIntervals; 
    vector<vector<RelationId> > pOrgsInIds;
    vector<vector<RelationId> > pOrgsAftIds;
    vector<vector<RelationId> > pRepsInIds;
    vector<vector<RelationId> > pRepsAftIds;
    vector<vector<RelationId> > pTmpOrgsIds;
    vector<vector<RelationId> > pTmpRepsIds;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pRepsInTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pOrgsInTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pOrgsAftTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pRepsAftTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pTmpOrgsTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pTmpRepsTimestamps;
    ...
    // Construction
    inline void updatePartitions(const Record &r);
    inline void updatePartitionsTmp(const Record &r);
    inline void clearPartitionsTmp(const Record &r);
public:
    // Construction
    Timestamp gstart, gend;
    Timestamp leafPartitionExtent;

    HINT_M_Dynamic_Naive(Timestamp leafPartitionExtent);
    HINT_M_Dynamic_Naive(const Relation &R, const unsigned int numBits, const unsigned int maxBits);
    void getStats();
    void print(char c);
    ~HINT_M_Dynamic_Naive() {};
    ...
    void insertTmp(RecordId id, Timestamp start);
    void expand();
    Timestamp removeTmp(RecordId id);
...
public:
    ...
    // Querying
    size_t execute_pureTimeTravel(RangeQuery Q);
    void execute_pureTimeTravel(RangeQuery Q, vector<RecordId> &candidates);
...
private:
    ...
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pOrgsAftTimestamps;
    vector<vector<vector<pair<Timestamp, Timestamp> > > > pRepsAftTimestamps;
    vector<vector<vector<int> > > pOrgsInAttrs;
    vector<vector<vector<int> > > pOrgsAftAttrs;
    vector<vector<vector<int> > > pRepsInAttrs;
    vector<vector<vector<int> > > pRepsAftAttrs;
...
public:
    ...
    HINT_M_Dynamic_Second_Attr(Timestamp leafPartitionExtent);
    HINT_M_Dynamic_Second_Attr(const Relation &R, const unsigned int numBits, const unsigned int maxBits);
    void getStats();
    void print(char c);
    ~HINT_M_Dynamic_Second_Attr() {};
    
    // Querying
    size_t execute_pureTimeTravel(RangeQuery Q);
    size_t execute_rangeTimeTravel(RangeQuery Q, int secondAttrLowerConstraint, int secondAttrUpperConstraint);
    size_t executeTimeTravel_greaterthan(RangeQuery Q, int secondAttrUpperConstraint);
    size_t executeTimeTravel_lowerthan(RangeQuery Q, int secondAttrLowerConstraint);
...
class HINT_M_Dynamic_undead : public HierarchicalIndex
{
private:
    vector<vector<RelationId> > pOrgsInIds;
    vector<vector<RelationId> > pOrgsAftIds;
...
public:
    ...
    HINT_M_Dynamic_undead(Timestamp leafPartitionExtent);
    HINT_M_Dynamic_undead(const Relation &R, const unsigned int numBits, const unsigned int maxBits);
    void getStats();
    void print(char c);
    ~HINT_M_Dynamic_undead() {};
    ...
    // Updating
    // bool checkDomain(Timestamp endpoint);
    void insert(const Record &r);
};

inline bool CompareIdTimestampPairsById(const pair<RecordId, Timestamp> &lhs, const pair<RecordId, Timestamp> &rhs)
{
